...
Current ESM readiness estimate: **~62%**.

E1/E2/E3/E4/E5.1/E5.2/E5.3/E5.4/E5.5/E5.6/E5.7 status:
...
- Build tooling readiness: **~86%** — Vite config, npm scripts, TypeScript checks, and build command are present and working.
- Frontend module boundary readiness: **~67%** — 19 extracted `app/` IIFE modules exist, 5 low-risk leaf helpers, 3 runtime adapters, and 1 UI island are importable.
- Runtime entry readiness: **~48%** — ESM can now access DOM/Store/appState/DB through adapters and one UI island, but `app.js`, AI/offline scripts, 204 inline handlers, and 31 local classic scripts still depend heavily on global script order.
...
- E5: inline handler cleanup requires island-by-island work and mobile QA because 204 inline handlers remain.
