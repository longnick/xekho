...
- `app.js`
  ...
  - used by: POS page.
  - notes: now explicitly denies stale `menu` / `insights` / `media` navigation after those obsolete pages were removed; `submitMenuItem()` uses Vietnamese money parsing for menu prices and an id-aware recipe gate so existing menu items can save price edits even if they currently have no recipe rows; `openBillModal()` uses `formatBillUnitPrice()` for the bill `Đ.Giá` column so fractional-thousand prices such as `17.500đ` display as `17,5K` instead of rounded `18K`; order header `#order-table-note` syncs quick table notes with cart/order extras and table notes; `renderTables()` also displays escaped full-width note text inside physical table cards; inspect diff before editing.
