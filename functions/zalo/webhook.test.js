...
function makeRequest({ method = 'POST', secret = 'zalo-secret-123', body, contentLength = '200' } = {}) {
  return {
    method,
    headers: {
...
describe('Zalo webhook boundary', () => {
  ...
  test('accepts a valid signed event, dispatches text without logging it, and logs metadata only', async () => {
    ...
    await createZaloWebhookHandler({ getSecret: () => 'zalo-secret-123', logger, onEvent })(makeRequest(), response);
  ...
  test('rejects invalid secret and malformed payload', () => {
    const logger = { info: jest.fn(), warn: jest.fn(), error: jest.fn() };
    const handler = createZaloWebhookHandler({ getSecret: () => 'zalo-secret-123', logger });
