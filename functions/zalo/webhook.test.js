...
function makeRequest({ method = 'POST', secret = '***REVOKED_CREDENTIAL***', body, contentLength = '200' } = {}) {
  return {
    method,
    headers: {
...
describe('Zalo webhook boundary', () => {
  ...
  test('accepts a valid signed event, dispatches text without logging it, and logs metadata only', async () => {
    ...
    await createZaloWebhookHandler({ getSecret: () => '***REVOKED_CREDENTIAL***', logger, onEvent })(makeRequest(), response);
  ...
  test('rejects invalid secret and malformed payload', () => {
    const logger = { info: jest.fn(), warn: jest.fn(), error: jest.fn() };
    const handler = createZaloWebhookHandler({ getSecret: () => '***REVOKED_CREDENTIAL***', logger });
    const invalidSecret = makeResponse();
    handler(makeRequest({ secret: 'wrong' }), invalidSecret);
