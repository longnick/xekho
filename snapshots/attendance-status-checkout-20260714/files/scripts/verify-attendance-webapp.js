...
// 4i) firestore.rules allow staff to create/update attendance, delete remains admin-only
const rulesJs = fs.readFileSync(require('path').join(root, 'firestore.rules'), 'utf8');
{
  // Extract attendance_daily rule block
  const dailyMatch = rulesJs.match(/match \/attendance_daily\/\{docId\}\s*\{([^}]+)\}/s);
  const dailyBlock = dailyMatch ? dailyMatch[1] : '';
  assert(
    /allow\s+create\s*,\s*update\s*:\s*if\s+isStaff/.test(dailyBlock) ||
    /allow\s+create\s*:\s*if\s+isStaff/.test(dailyBlock) ||
    /allow\s+create\s*,\s*update\s*:\s*if\s+(isAdmin|isStaff)\(\)|allow\s+write\s*:\s*if\s+isStaff/.test(dailyBlock),
    'firestore.rules attendance_daily allows create/update for isStaff()'
  );
  assert(
...
}
{
  ...
  assert(
    /allow\s+create\s*,\s*update\s*:\s*if\s+isStaff/.test(shiftsBlock) ||
    /allow\s+create\s*:\s*if\s+isStaff/.test(shiftsBlock) ||
    /allow\s+write\s*:\s*if\s+isStaff/.test(shiftsBlock),
    'firestore.rules attendance_shifts allows create/update for isStaff()'
  );
  assert(
...
assert(
  indexHtml.includes('app.js?v=20260714-attendance-status-checkout'),
  'index.html cache key is bumped for the status-bar checkout UI'
);
